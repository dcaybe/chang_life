...
class FoodVM extends AutoDisposeAsyncNotifier<List<Food>> {
  ...
  Future<List<Food>> build() async {
    ...
    ref.onCancel(() {
      print('⏳ [FoodVM] onCancel: Không ai xem nữa, bắt đầu đếm ngược 10s để xóa cache...');
      Timer(const Duration(seconds: 10), () {
        print('🗑️ [FoodVM] Timer: Đã hết 10s, chính thức xóa cache (dispose)!');
    ...
    if (cachedFoods != null && cachedFoods.isNotEmpty) {
      print('📦 [FoodVM] Lấy thành công dữ liệu từ Hive Cache! Không gọi API nữa.');
      return cachedFoods;
    }
